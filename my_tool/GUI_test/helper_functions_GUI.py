...
def get_array(saving_path, key):
    print("[DEBUG] Reading CSV in get_array from:", saving_path)
    df = pd.read_csv(saving_path)
    print("[DEBUG] Columns:", df.columns.tolist())
    vector = df[key].values
    result_array = np.array(vector)
    print("[DEBUG] Returned array of length:", len(result_array))
    return result_array
...
def plot_all(
    ...
    if gen_range:
        print("[DEBUG_02] Applying gen_range filtering")
    ...
    print("[DEBUG_03] Creating figure")
    ...
    print("[DEBUG_04] Adding subplots")
    ...
    print("[DEBUG_05] Calling plot_pressure_curves()")
    ...
    print("[DEBUG_06] Calling plot_error()")
    # Error plot
    plot_error(ax2, iteration_errors)

    print("[DEBUG_07] Plotting parameter evolution")
    ...
    plt.tight_layout()
    print(f"[DEBUG_08] Saving figure to {plot_dir}")
    plt.savefig(plot_dir, dpi=300)
    print("[DEBUG_09] Plot saved successfully")

    try:
        if show_plot:
            print("[DEBUG_10] Showing plot interactively")
            plt.show()
    except Exception as e:
        print("[ERROR] plt.show() failed:", e)
    print("[DEBUG_11] plot_all completed")
...
def get_best_curve(metadata_csv_path, pressure_curves_csv_path, best_x, time_step=0.1):
    ...
    print(f"[DEBUG] First 3 values of best pressure curve at index {match_idx}: {final_sim_pressure[:3]}")
