...
import shutil
import matplotlib
# Only set TkAgg if the backend is not already set
# if matplotlib.get_backend() != "TkAgg":
#     try:
#         matplotlib.use("TkAgg")
#     except ImportError:
#         print("[WARN] Could not use TkAgg backend (GUI not available).")
...
#     # Modify grain parameters based on type
#     for i, grain in enumerate(motor_data["grains"]):
#         grain_type = grain.get("type", "")

#         if grain_type == "BATES":
#             if "length1" in params:
#                 motor_data["grains"][i]["properties"]["length"] = float(params["length1"])
#             if "coreDiameter" in params:
#                 motor_data["grains"][i]["properties"]["coreDiameter"] = float(params["coreDiameter"])
#             if "diameter" in params:
#                 motor_data["grains"][i]["properties"]["diameter"] = float(params["diameter"])

#         elif grain_type == "Star Grain":
#             if "length2" in params:
#                 motor_data["grains"][i]["properties"]["length"] = float(params["length2"])
#             if "diameter" in params:
#                 motor_data["grains"][i]["properties"]["diameter"] = float(params["diameter"])
#             if "pointLength" in params:
#                 motor_data["grains"][i]["properties"]["pointLength"] = float(params["pointLength"])
#             if "pointWidth" in params:
#                 motor_data["grains"][i]["properties"]["pointWidth"] = float(params["pointWidth"])
#             if "numPoints" in params:
#                 motor_data["grains"][i]["properties"]["numPoints"] = int(params["numPoints"])

#     # Modify nozzle parameters
#     if "throat" in params:
#         motor_data["nozzle"]["throat"] = float(params["throat"])
...
def update_motor_file(motor_file, **params):
    ...
    for key, value in params.items():
        # print(f"[DEBUG] -------------------------------------------------")
        if key == "length1":
            # Replace the first occurrence of "length:"
            # print(f"[DEBUG] Replacing first occurrence of 'length:' with {value}")
            pattern = r"(length:\s*)([0-9.eE+-]+)"
            count = 0
            def replacer1(m):
                nonlocal count
                count += 1
            ...
            # Replace the second occurrence of "length:"
            pattern = r"(length:\s*)([0-9.eE+-]+)"
            count = 0
            def replacer2(m):
                nonlocal count
            ...
            def replacer3(m):
                ...
                if count == 3:
                    return m.group(1) + str(value)
                return m.group(0)
            content = re.sub(pattern, replacer3, content)

        elif key == "coreDiameter1":
            # Replace the first occurrence of "length:"
            # print(f"[DEBUG] Replacing first occurrence of 'length:' with {value}")
            pattern = r"(coreDiameter:\s*)([0-9.eE+-]+)"
            count = 0
            def replacer1(m):
                nonlocal count
                count += 1
        ...
        elif key == "pointLength1":
            pattern = r"(pointLength:\s*)([0-9.eE+-]+)"
            count = 0
            def replacer1(m):
                nonlocal count
                count += 1
        ...
        elif key == "pointWidth1":
            pattern = r"(pointWidth:\s*)([0-9.eE+-]+)"
            count = 0
            def replacer1(m):
                nonlocal count
                count += 1
            ...
            # Generic replacement for unique parameters
...
def plot_pressure_curves(ax, pressures, times, time_vector, target_pressure, final_time, final_pressure,
                         best_x, best_f, pop_size, optimization_variables, yellow_end=10, red_end=50, show_plot=False):
    ...
    # Best solution info (removed: now displayed as external table)
...
def plot_all(
    ...
    max_params_per_row = 4
    param_rows = int(np.ceil(num_vars / max_params_per_row))
    total_rows = 1 + param_rows + 1  # 1 for top row (pressure + error), rest for parameters, extra row for table
    total_cols = max(4, max_params_per_row)

    fig = plt.figure(figsize=(5 * total_cols, 5 * total_rows), dpi=100, constrained_layout=True)
    gs = gridspec.GridSpec(total_rows, total_cols, hspace=0.5, wspace=0.4)

    # Top row: pressure and error plots
    ax1 = fig.add_subplot(gs[0, :2])
    ax2 = fig.add_subplot(gs[0, 2:4])

    # Parameter evolution plots (spread across multiple rows if needed)
    param_axes = []
    for i in range(num_vars):
        row = 1 + (i // max_params_per_row)
        col = i % max_params_per_row
    ...
    for i, (name, color) in enumerate(zip(param_names, param_colors)):
        ...
        plot_param_evolution(param_axes[i], values, f"{name}", name, color=color)
    ...
    # Place the table in the last row using gridspec, spanning all columns
    table_ax = fig.add_subplot(gs[-1, :])
    table_ax.axis('off')

    # Prepare data
    cell_text = [[var['label'].split()[0], f"{val:.5f}"] for var, val in zip(optimization_variables, best_x)]
    cell_text.append(["Error", f"{best_f[0]:.2e}"])

    # Create and add table
    table = table_ax.table(cellText=cell_text, colLabels=["Variable", "Value"], loc='center')
...
#     import matplotlib.pyplot as plt
#     import matplotlib.gridspec as gridspec
#     from optimizer_GUI import plot_pressure_curves, plot_error, plot_param_evolution

#     print("[DEBUG_01] plot_all started")
...
#     # --- Match best_x in metadata to get index ---
#     df_meta = pd.read_csv(metadata_csv_path)
#     param_cols = df_meta.columns[3:]  # Skip Iteration, Error, Sim_Success
#     df_params = df_meta[param_cols]
#     best_x_rounded = np.round(best_x, 5)

#     match_idx = None
#     for idx, row in df_params.iterrows():
#         row_rounded = np.round(row.values.astype(float), 5)
#         if np.array_equal(row_rounded, best_x_rounded):
#             match_idx = idx
#             break

#     if match_idx is None:
#         raise ValueError("Best_x parameters not found in metadata file.")

#     print(f"[DEBUG] Matched best_x at index {match_idx}")

#     # --- Load pressure curve using manual CSV reading ---
#     with open(pressure_curves_csv_path, 'r') as f:
#         lines = f.readlines()
...
#             try:
#                 val = float(values[match_idx + 1])
#                 final_sim_pressure.append(val)
#             except (IndexError, ValueError):
#                 continue
#     print(f"[DEBUG] First 3 values of matched pressure curve at index {match_idx}: {final_sim_pressure[:3]}")

#     final_sim_time = list(range(len(final_sim_pressure)))
...
#     ax1 = fig.add_subplot(gs[0, :2])
#     ax2 = fig.add_subplot(gs[0, 2:4])
#     param_axes = [fig.add_subplot(gs[1, i]) for i in range(num_vars)]
...
#     plt.tight_layout()
#     print(f"[DEBUG] Saving figure to {plot_dir}")
#     plt.savefig(plot_dir, dpi=300)
#     if show_plot:
#         try:
#             plt.show()
...
#     print("[DEBUG] plot_all completed")
...
def plot_bates_geometry(diameter, core_diameter, show_plot=False, save_path=None):
    """
...
def plot_endburner_geometry(diameter, show_plot=False, save_path=None):
    """
...
def generate_star_geometry_from_scratch_2(diameter, pointLength, pointWidth, numPoints, show_plot=False, save_path=None):
    """
...
def callback_update(*args, **kwargs):
    print("[INFO] Deprecated callback_update called. Use progress_queue instead.")

# The global GUI instance (set by gui_launcher.py)
# gui_instance = None
# === UTILITY FUNCTION TO GET BEST CURVE ===
def get_best_curve(metadata_csv_path, pressure_curves_csv_path, best_x, time_step=0.1):
    import numpy as np
    import pandas as pd
    ...
    with open(pressure_curves_csv_path, 'r') as f:
        ...
        if match_idx >= len(lines):
            raise IndexError("Matched index exceeds number of pressure curve rows")
        selected_line = lines[match_idx].strip()
        final_sim_pressure = [float(val) for val in selected_line.split(',') if val.strip() != '']

    final_sim_time = [i * time_step for i in range(len(final_sim_pressure))]

    return final_sim_time, final_sim_pressure