...
class PressureCurveFitProblem:
    ...
    def fitness(self, x):
        ...
        print("------------------------")
        print("--------------------------")
        # --- Progress update: Use progress_queue if available ---
        # Instead of callback_update, use progress_queue for GUI
        if 'progress_queue' in globals() and globals()['progress_queue'] is not None:
            pq = globals()['progress_queue']
            if pq:
                pq.put((progress_counter, expected_evaluations))
        return [error]
